...
# Replace lines around 200-260 with this properly indented version:
...
if not df.empty:
    ...
    custom_status = st.sidebar.text_input("Enter custom status names (comma-separated)", 
                                         placeholder="e.g., Status 2, Status 3",
                                         key="custom_status_input")

    # Add button to apply custom statuses
    if st.sidebar.button("Add Custom Statuses", key="add_custom_status"):
        if custom_status:
            custom_statuses = [s.strip() for s in custom_status.split(",") if s.strip()]
            if custom_statuses:
                # Add custom statuses to the status_stage list
                status_stage.extend(custom_statuses)
                st.sidebar.success(f"Added {len(custom_statuses)} custom status(es): {', '.join(custom_statuses)}")
            else:
                st.sidebar.warning("Please enter valid status names")
        else:
            st.sidebar.warning("Please enter status names before adding")

    # Show current status filters
    if status_stage:
        with st.sidebar.expander("Current Status Filters"):
            for status in status_stage:
                st.write(f"• {status}")

    # You can also add a "Clear Custom Statuses" button if needed
    if st.sidebar.button("Reset Status Filters", key="reset_status"):
        status_stage = all_status_stages.copy()
        st.sidebar.success("Status filters reset to default")
        st.rerun()
